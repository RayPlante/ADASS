...
After that initial production period the archive went into maintenance mode and would automatically re-trigger the processing of datasets which would be touched by changes in reference files of \ssindex{data!pipelines!reduction}pipeline software just like the active \ssindex{archives!individual!HST Archive}\ssindex{observatories!space-based!HST}HST archive at ESO did at the same time.
