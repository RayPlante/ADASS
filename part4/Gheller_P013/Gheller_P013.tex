...
We employed a medium-sized cosmological \ssindex{simulations!N-body}N-body \ssindex{astronomy!simulation}simulation performed using the Gadget code (\citet{springel2005}). The dataset contains about 400 million particles, describing \ssindex{astronomy!darm matter}dark matter ($\approx 200$ million), baryonic matter (same amount) and stars ($\approx 10$ million). Figure~\ref{fig:box} shows a section of the data cube visualized by \ssindex{algorithm!Splotch}Splotch. Our tests have been performed on a 16-core AMD Opteron 6272 2.1 GHz Interlagos processor, equipped with an \ssindex{NVIDIA}NVIDIA Tesla X2090 \ssindex{computing!GPU}GPU with 6 GB of GDDR5 memory. We employed GCC 4.3.4 and CUDA 5.0.20 versions for code compilation.
