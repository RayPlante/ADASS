...
Figure~\ref{bdiagram} shows a block diagram of our \ssindex{computing!cloud}cloud-based workflow, which consists of two MapReduce jobs, a pre-processing phase, and a post-processing phase. To implement the MapReduce jobs, we use the \ssindex{packages!Hadoop}Hadoop MapReduce framework. In particular, since we already have a binary executable for the \ssindex{applications!SExtractor}SExtractor program (Bertin \& Arnouts 1996), we chose the \ssindex{packages!Hadoop}Hadoop Streaming framework. \ssindex{packages!Hadoop}Hadoop streaming is a framework that is included in the \ssindex{packages!Hadoop}Hadoop distribution that allows us to create and run MapReduce jobs with any executable or script as the mapper and/or the reducer. Our workflow is written by using a mix of \ssindex{computer languages!Java}Java and \ssindex{computer languages!Python}Python programs and shell scripts, as detailed below.
