...
\markboth{Kenny J. Glotfelty}{One Hop: In The Right Direction}
...
\articlefigure{I04_f1.eps}{FigOne}{Simplified view of a Chandra dataset.  The primary science product is the event file which contains multiple FITS extensions:  one event table with multiple rows and one or more Good Time Interval (GTI) tables associated with each active CCD.  There are several other auxiliary files that are required to do analysis such as the aspect solution (pointing history), detector mask file defining the active regions, and various other instrument level products that describe the configuration of each detector element.}
