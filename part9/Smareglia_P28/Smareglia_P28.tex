...
\articlefigure[]{part9/Smareglia_P28/P028_f1.eps}{fig:dbpipeline}{PESSTO data survey files, incoming from an observation through the Marshall, are ingested according to the pipeline structure into a relational MySQL database while FITS metadata are stored in a set of observational and technical DB tables. Relationships also exist to raw data files (available from ESO archives). The main exposed section of the relational DB are the final objects (scientific data) output of the pipeline processing.}
