...
\item[SOFIA Proposal Tool (SPT)] \ssindex{observatories!Earth-based!SOFIA}SOFIA proposals are created, modified, and submitted to the SSC using SPT, a stand-alone \ssindex{computer languages!Java}Java application available for a wide variety of platforms based on the Astronomers Proposal Tool (APT)\footnote{
\url{http://www.stsci.edu/hst/proposing/apt}}
 developed at STScI for \ssindex{observatories!space-based!HST}HST.


\item[SOFIA-Spot] A stand-alone \ssindex{computer languages!Java}Java application that allows the GI to create, modify, and submit AORs for his or her observing program as part of the Phase~II planning process. S-SPOT provides a unified interface for configuring observations with the various \ssindex{observatories!Earth-based!SOFIA}SOFIA SIs; graphical overlays for visualizing an observation on a background image; and integrated access to existing IR survey mission archives (\ssindex{observatories!Earth-based!2MASS}2MASS, MSX, \ssindex{observatories!space-based!IRAS}IRAS, etc...).  S-Spot is based on the Spot tool\footnote{
\url{http://ssc.spitzer.caltech.edu/warmmission/propkit/spot/}}
developed for the \ssindex{observatories!space-based!Spitzer}Spitzer Space Telescope.
...
\item[Observation \ssindex{software!observation planning}Planning System] The \ssindex{software!observation planning}planning system is integrated with both SPT and SSpot to support the Phase~I and Phase~II planning activities.  \ssindex{software!observation planning}Planning system data is organized around {\bf observing plans} (proposals), which are composed of {\bf AOR}s.  Planning data is made available to external clients ~\citep[e.g. Flight Management Infrastructure,][]{Gross:2009} using a well-defined API.  
...
Over 170 proposals were submitted during Phase~I using SPT.  S-Spot was successfully used during Phase~II planning by the Cycle~1 GIs for creating and submitting AORs (a process that was done interactively with the GIs during Basic Science).  There are now $\approx 800$ AORs  stored in the DCS that are currently being used by the flight \ssindex{software!scheduling}scheduling and \ssindex{software!observation planning}planning tools.
...
In addition, the DCS archive contains over 850~GB of mission data (guide camera images, housekeeping data, etc...) and other \ssindex{data!ancillary}ancillary data products (mission audio, logs, etc...).  
